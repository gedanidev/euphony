...
import { getArtists, createArtist, deleteArtist, toggleArtistPreferred } from '../api/artists'
...
export default function Artists() {
  ...
  const [page, setPageState] = useState(() => parseInt(sessionStorage.getItem('artists-page') || '1', 10))
  const setPage = (v) => setPageState(prev => {
    const next = typeof v === 'function' ? v(prev) : v
    sessionStorage.setItem('artists-page', String(next))
    return next
  })
  ...
  const load = async () => {
    try {
      ...
      const data = await getArtists({ search: search || undefined, letter: letter || undefined, page, limit })
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex items-center gap-4 mb-6">
        <input type="text" placeholder={t('artists.search')} value={search} onChange={e => { setSearch(e.target.value); setLetter(null) }}
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
            {artists.map(artist => (
              <div key={artist.id} onClick={() => navigate(`/artists/${artist.id}`)}
                ...
                <button onClick={e => handlePreferred(e, artist.id)}
                  className={`absolute top-2 left-2 text-xl transition-colors ${artist.is_preferred ? 'text-amber-400' : 'text-[#3d3d5c] hover:text-amber-300'}`}
          ...
          <div className="bg-[#1a1a24] border border-[#2e2e4a] rounded-xl overflow-hidden">
            {artists.map((artist, idx) => (
              <div key={artist.id}
                ...
                className={`flex items-center gap-4 px-4 py-3 cursor-pointer hover:bg-[#22223a]/60 transition-colors group ${idx < artists.length - 1 ? 'border-b border-[#2e2e4a]' : ''}`}>
                <div className="w-10 h-10 rounded-full bg-[#22223a] flex-shrink-0 flex items-center justify-center overflow-hidden">
                  {artist.image_url ? (
                    <img src={artist.image_url} alt={artist.name} className="w-full h-full object-cover" />
                  ) : (
                    <svg className="w-5 h-5 text-[#2e2e4a]" fill="currentColor" viewBox="0 0 24 24">
                      <path d="M12 12c2.7 0 4.8-2.1 4.8-4.8S14.7 2.4 12 2.4 7.2 4.5 7.2 7.2 9.3 12 12 12zm0 2.4c-3.2 0-9.6 1.6-9.6 4.8v2.4h19.2v-2.4c0-3.2-6.4-4.8-9.6-4.8z"/>
                    </svg>
                  )}
                </div>
                <div className="flex-1 min-w-0">
                  ...
                  {(artist.country || artist.region) && (
                    <p className="text-xs text-[#94a3b8] truncate">
                      {[artist.country, artist.region].filter(Boolean).join(' · ')}
                    </p>
                  )}
                </div>
                <button onClick={e => handlePreferred(e, artist.id)}
                  className={`text-lg transition-colors flex-shrink-0 ${artist.is_preferred ? 'text-amber-400' : 'text-[#3d3d5c] hover:text-amber-300'}`}
                  title={artist.is_preferred ? t('artist.removePreferred') : t('artist.addPreferred')}>
                  {artist.is_preferred ? '★' : '☆'}
                </button>
                <button onClick={e => handleDelete(e, artist.id)}
