  ...
  return (
    <div className="p-6">
      ...
      <div className="flex items-center gap-4 mb-6">
        <input type="text" placeholder="Buscar artistas…" value={search} onChange={e => setSearch(e.target.value)}
        ...
        {total > 0 && <span className="text-[#94a3b8] text-sm">{total} artistas</span>}
      </div>
      ...
      {!loading && !error && artists.length === 0 && (
        <EmptyState title="No hay artistas" description="Crea tu primer artista o importa una playlist M3U8"
          action={<button onClick={() => setShowCreate(true)} className="px-4 py-2 bg-purple-600 hover:bg-purple-700 text-white rounded-lg text-sm transition-colors">Crear Artista</button>} />
      )}
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-center gap-3 mt-6">
              <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1}
                className="px-3 py-1 rounded text-sm disabled:opacity-40 text-[#94a3b8] hover:text-white transition-colors">← Anterior</button>
              <span className="text-[#94a3b8] text-sm">{page} / {totalPages}</span>
              <button onClick={() => setPage(p => Math.min(totalPages, p + 1))} disabled={page === totalPages}
                className="px-3 py-1 rounded text-sm disabled:opacity-40 text-[#94a3b8] hover:text-white transition-colors">Siguiente →</button>
            </div>
          )}
        </>
      )}
